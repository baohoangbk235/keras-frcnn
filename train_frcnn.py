...
for epoch_num in range(num_epochs):
	...
	while True:
		...
		X, Y, img_data = next(data_gen_train) #X shape: (1, 416, 1077, 3)

		loss_rpn = model_rpn.train_on_batch(X, Y) 

		P_rpn = model_rpn.predict_on_batch(X) #(1, 26, 68, 9) (1, 26, 68, 36)


		R = roi_helpers.rpn_to_roi(P_rpn[0], P_rpn[1], C, K.common.image_dim_ordering(), use_regr=True, overlap_thresh=0.5, max_boxes=300)
